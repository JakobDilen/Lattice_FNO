...
# General settings
BASE_FOLDER = "assets"
MODELS = ["normal", "lattice", "hyperbolic_cross_normal", "hyperbolic_cross_lattice"]
MODEL_SEEDS = [
...
def main(base_folder=BASE_FOLDER, base_data_folder=BASE_FOLDER):
    for model_seeds in MODEL_SEEDS:
        ...
        experiment_folder = f"{base_folder}/results/experiment_general_analysis-step-{SCHEDULER_STEP}-factor-{SCHEDULER_FACTOR}-lr-{INIT_LR}-m-{M}-alpha-{ALPHA}-xi-{XI}"

        print("TRAIN MODELS \n ")
        for model_type in MODELS:
            general_analysis.main(
                train="True",
                analyse="False",
