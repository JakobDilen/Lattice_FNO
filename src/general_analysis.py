"""
This file runs the experiment comparing the original FNO to both lattice variations.
"""
...
def trainModels(
    ...
    base_experiment_folder = f"{base_folder}/general_analysis"
        ...
        for n_layers in N_LAYERS:
            for n_channels in N_CHANNELS:
                ...
                if "normal" in model_types:
                    ...
                    filename = f"{experiment_folder}/normal-model_seed-{model_seeds[0]}/logs.txt"
                    np.savetxt(filename, normal_time_results, fmt="%s")

                    filename = f"{experiment_folder}/normal-model_seed-{model_seeds[0]}/lr.txt"
                ...
                if "lattice" in model_types:
                    ...
                    filename = f"{experiment_folder}/lattice-model_seed-{model_seeds[1]}/logs.txt"
                    np.savetxt(filename, lattice_time_results, fmt="%s")

                    filename = f"{experiment_folder}/lattice-model_seed-{model_seeds[1]}/lr.txt"
                    np.savetxt(filename, model_stats[2], fmt="%s")
                ...
                if "hyperbolic_cross_normal" in model_types:
                    ...
                    filename = f"{experiment_folder}/hyperbolic_cross_normal-model_seed-{model_seeds[0]}/logs.txt"
                    np.savetxt(filename, hyperbolic_cross_normal_time_results, fmt="%s")

                    filename = f"{experiment_folder}/hyperbolic_cross_normal-model_seed-{model_seeds[0]}/lr.txt"
                    np.savetxt(filename, model_stats[2], fmt="%s")
                ...
                if "hyperbolic_cross_lattice" in model_types:
                    ...
                    filename = f"{experiment_folder}/hyperbolic_cross_lattice-model_seed-{model_seeds[2]}/logs.txt"
                    np.savetxt(filename, hyperbolic_cross_lattice_time_results, fmt="%s")

                    filename = f"{experiment_folder}/hyperbolic_cross_lattice-model_seed-{model_seeds[2]}/lr.txt"
                    np.savetxt(filename, model_stats[2], fmt="%s")
            ...
            for M in M_list:
                ...
                if "hyperbolic_cross_lattice" in model_types:
                    ...
                    errorCalculationScript(
                        ...
                        use_lattice=True,
                        data_folder=data_experiment_folder,
                        save_path=f"{experiment_folder}/hyperbolic_cross_lattice-model_seed-{model_seeds[2]}",
                        model_folder=f"{experiment_folder}/hyperbolic_cross_lattice-model_seed-{model_seeds[2]}",
                        start_id=iteration,
                        stop_id=iteration + 1,
                        save_epochs=np.arange(0, EPOCHS + 1, INTERVAL),
                        train_seed=TRAIN_SEED[iteration],
...
if __name__ == "__main__":
    ...
    if len(arguments) == 4:
        ...
        iteration = "0"
    if randomise == "True":
        TRAIN_SEED, TEST_SEED, GENERALIZATION_SEED = np.random.randint(0, 1000000000, 3)
        if TRAIN_SEED == TEST_SEED:
            TEST_SEED += 1
        if TRAIN_SEED == GENERALIZATION_SEED:
            GENERALIZATION_SEED += 1
        if TEST_SEED == GENERALIZATION_SEED:
            GENERALIZATION_SEED += 1
